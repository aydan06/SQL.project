...
FROM customer_orders
 ...
 --3.Which customers ordered the most amount of rolls and which kind?

SELECT customer_id,roll_name,rolls_ordered FROM (SELECT customer_id, roll_name,rolls_ordered, RANK() OVER(ORDER BY rolls_ordered DESC) FROM
...
--4. How many successful orders delivered by each driver?

SELECT driver_id, SUM(orders_delivered) successful_orders
FROM (SELECT driver_id, COUNT(DISTINCT(order_id)) orders_delivered,
CASE WHEN cancellation LIKE '%Cancellation%' THEN 'c' ELSE 'nc'
END cancellation_details 
FROM driver_order
GROUP BY driver_id, cancellation) a
WHERE cancellation_details = 'nc'
GROUP BY driver_id




--5.How many of each type of roll was successifully delivered?

SELECT roll_name, SUM(rolls_delivered )
FROM (SELECT r.roll_name, COUNT(*) rolls_delivered,
CASE WHEN cancellation LIKE '%Cancellation%' THEN 'c' ELSE 'nc'
END cancellation_details
FROM rolls r  
JOIN customer_orders ON r.roll_id=c.roll_id
JOIN driver_order d  ON d.order_id = c.order_id
GROUP BY roll_name, cancellation) a
WHERE cancellation_details = 'nc'
GROUP BY roll_name



--6. How many Veg & Nonveg rolls were ordered by each customer?
...
SELECT MAX(rolls_delivered) max_rolls_delivered FROM (SELECT c.order_id, COUNT(r.roll_id) rolls_delivered,
CASE WHEN d.cancellation LIKE '%Cancellation%' THEN 'c' ELSE 'nc'
END cancellation_details
...
(SELECT order_id,customer_id,roll_id, CASE WHEN 
 not_include_items is null OR not_include_items ='' THEN '0' ELSE not_include_items
END not_include_clean,
CASE 
WHEN extra_items_included is null OR extra_items_included ='' OR extra_items_included = 'NaN' THEN '0' ELSE extra_items_included 
END extra_items_clean, order_date
FROM customer_orders),
...
WHEN cancellation LIKE '%Cancellation%' THEN 0 ELSE 1 END new_cancelation
FROM driver_order)

SELECT customer_id,SUM(rolls)at_least_1_change,chng_no_chng
FROM 
...
SELECT chng_no_chng, SUM(rolls) rolls_delivered FROM (SELECT r.roll_id, COUNT(c.order_id) rolls, 
CASE WHEN c.not_include_items<>'0' AND c.extra_items_included<>'0' THEN 'both inc ex' ELSE 'either 1 ex or inc' END chng_no_chng
FROM rolls r 
JOIN temp_cust_orders c ON r.roll_id=c.roll_id
JOIN temp_driver_order d ON d.order_id=c.order_id
...
SELECT hours_bucket, COUNT(*) rolls_ordered FROM
...
SELECT TO_CHAR(order_date, 'Day') weekday, COUNT(DISTINCT order_id)
FROM customer_orders
GROUP BY weekday
